...
public class CommonFun {
	...
	public static void ClickWebElement(WebDriver driver, By byxpath) {
	...
	public void ClearWebElement(String txt) {
		driver.findElement(By.xpath(txt)).clear();
	}

	public void enterData(String path, String txt) {
		driver.findElement(By.xpath(path)).sendKeys(txt);
	}

	public static void waitForElement(WebDriver driver, String str) {
		WebElement elewait = driver.findElement(By.xpath(str));
	...
	public static void scrollme(WebDriver driver) {
		int last=500; int i,j=0;
		JavascriptExecutor js = (JavascriptExecutor) driver;
		  for (i = 200; j <= last; i += 100) { 
			  j=i+100;
		  }
	...
	public static void photo(String filename) throws AWTException {
		...
		try {
			...
			// Create a file name
			String Path = System.getProperty("user.dir") + "//test-output//Screenshot//" + filename
	...
	public static void CountLinks(WebDriver driver, String xpath) {
		...
		// List<WebElement> links = driver.findElements(By.xpath("//img[@class='_46-i
		// img']"));
		count = links.size();
				...
				if (count > 5) {
					try {
						...
						if (httpURLConnect.getResponseCode() >= 400) {
							Reporter.log(
									url + " HTTP code: " + responseCode + "=" + httpURLConnect.getResponseMessage());
						...
						// This catch block handles exceptions like MalformedURLException or
						// IOExceptions
	...
	public static void checkBrokenLink(String linkUrl) {
		...
		try {
			...
			if (httpURLConnect.getResponseCode() >= 400) {
				Reporter.log(linkUrl + " is a broken link." + " HTTP status code: " + responseCode + "="
